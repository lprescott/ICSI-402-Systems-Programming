...
printf("\"%d\"\n", tempDepartureYear);
	...
	fwrite(&tempFlight->AirlineCodeAndFlightNumber[0], sizeof(tempFlight->AirlineCodeAndFlightNumber), 1, flightData);
	fwrite(&space, sizeof(space), 1, flightData);

	fwrite(&tempFlight->OriginAirportCode[0], sizeof(tempFlight->OriginAirportCode), 1, flightData);
	fwrite(&space, sizeof(space), 1, flightData);

	fwrite(&tempFlight->DestinationAirportCode[0], sizeof(tempFlight->AirlineCodeAndFlightNumber), 1, flightData);
	fwrite(&space, sizeof(space), 1, flightData);

	fwrite(&tempFlight->DepartureDayOfWeek[0], sizeof(tempFlight->DepartureDayOfWeek), 1, flightData);
	fwrite(&space, sizeof(space), 1, flightData);

	fwrite(&tempFlight->DepartureMonth[0], sizeof(tempFlight->DepartureMonth), 1, flightData);
	fwrite(&space, sizeof(space), 1, flightData);

	fwrite(&tempFlight->DepartureDay, sizeof(tempFlight->DepartureDay), 1, flightData);
	fwrite(&space, sizeof(space), 1, flightData);

	fwrite(&tempFlight->DepartureTime[0], sizeof(tempFlight->DepartureTime), 1, flightData);
	fwrite(&space, sizeof(space), 1, flightData);

	fwrite(&tempFlight->DepartureYear, sizeof(tempFlight->DepartureYear), 1, flightData);
	fwrite(&space, sizeof(space), 1, flightData);


      free(tempFlight);

      printf("\nPlease enter the next flag:\n");

    }
    else if(input == 2){
        //(c) Read and print info about all flight records in the file
        printDetails(flightData);
    }
  }
